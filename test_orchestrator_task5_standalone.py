...
def _intent_cases():
    return [
        ...
        ("parse_function_with_to", "Create a function to normalize", "function", "normalize"),
        ...
        ("detect_recursive_constraint", "Create function factorial recursive", "function", "factorial"),
        ("detect_async_constraint", "Create function fetch async", "function", "fetch"),
        ("detect_error_handling_requirement", "Create function parse and handle errors", "function", "parse"),
        ("detect_docstring_requirement", "Create function sum with docstring", "function", "sum"),
        ("detect_tests_requirement", "Create function hash and test it", "function", "hash"),
        ("detect_flask_dependency", "Create flask api route", "module", "api_module"),
        ("detect_fastapi_dependency", "Create fastapi endpoint", "module", "api_module"),
        ("detect_pandas_dependency", "Create pandas data cleaner", "module", "generated_artifact"),
        ("complexity_low", "Create simple quick helper function to trim", "function", "trim"),
        ("complexity_high", "Create complex distributed scalable orchestrator", "module", "generated_artifact"),
...
for case_name, intent, expected_type, expected_target in _intent_cases():
    def _make_test(_intent=intent, _expected_type=expected_type, _expected_target=expected_target):
        def _test(self):
            parser = IntentParser()
...
def _plan_cases():
    return [
        ...
        ("test_file_derivation", _make_parsed(target="alpha"), None, "generated/alpha.py"),
        ("effort_low", _make_parsed(complexity="low"), None, "generated/do_work.py"),
        ("effort_medium", _make_parsed(complexity="medium"), None, "generated/do_work.py"),
        ...
        ("parsed_intent_roundtrip", _make_parsed(target="beta"), None, "generated/beta.py"),
        ("class_target_generation", _make_parsed(intent_type="class", target="Widget"), None, "generated/Widget.py"),
        ("api_scope_preserved", _make_parsed(intent_type="module", target="api"), ["aetherra_hub/new_api.py"], "aetherra_hub/new_api.py"),
        ("derive_test_file_custom_scope", _make_parsed(target="x"), ["pkg/mod.py"], "pkg/mod.py"),
...
for case_name, parsed, scope, expected_target in _plan_cases():
    def _make_test(_parsed=parsed, _scope=scope, _expected_target=expected_target, _case=case_name):
        def _test(self):
            gen = PlanGenerator()
            ...
            if _case == "steps_names_order":
                ...
                self.assertEqual(names, ["skeleton", "logic", "errors", "docs", "tests"])
...
def _workflow_cases():
    return [
        ...
        ("async_skeleton", ParsedIntent("Create function fetch async", "function", "fetch", constraints=["async"])),
        ...
        ("class_tests_reference_class", _make_parsed(intent_type="class", target="Widget")),
        ("function_tests_reference_function", _make_parsed(target="echo")),
        ("docs_include_constraints", ParsedIntent("x", "function", "x", constraints=["recursive"])),
        ("docs_include_dependencies", ParsedIntent("x", "function", "x", dependencies=["flask"])),
        ("recursive_constraint_preserved", ParsedIntent("x", "function", "factorial", constraints=["recursive"])),
...
def _orchestrator_cases():
    return [
        ("plan_creates_steps", "Create function factorial recursively", ["generated/factorial.py"]),
        ("plan_without_scope", "Create function add", None),
        ("generate_step0_dry_run", "Create function normalize", ["generated/normalize.py"]),
        ("generate_invalid_step_index_raises", "Create function normalize", ["generated/normalize.py"]),
        ("plan_stores_internal_parsed_intent", "Create function hash", ["generated/hash.py"]),
        ("plan_sets_codegen_plan", "Create class ItemBuilder", ["generated/item_builder.py"]),
        ("generate_for_new_file_contains_def", "Create function sanitize", ["generated/sanitize.py"]),
        ("generate_summary_contains_target", "Create function clamp", ["generated/clamp.py"]),
        ("plan_complexity_high", "Create complex distributed orchestrator", ["generated/complex_orch.py"]),
        ...
        def _test(self):
            with tempfile.TemporaryDirectory() as td:
                ...
                if _case == "plan_without_scope":
                    self.assertTrue(orch._codegen_plan.target_file.startswith("generated/"))
                ...
                if _case == "plan_logs_steps":
                    self.assertTrue(any("skeleton" in s.description for s in plan.steps))
        return _test
