                ...
                if file.endswith('.py'):
                    ...
                    try:
                        ...
                        if unicode_chars:
                            ...
                            total_count = sum(content.count(char) for char in unicode_chars)
                            unicode_files.append((file_path.relative_to(ROOT_DIR), total_count, unicode_chars))
...
def main():
    parser = argparse.ArgumentParser(description="Fix Aetherra workflow failures automatically")
    parser.add_argument('--dry-run', action='store_true', 
                       help='Show what would be changed without making changes')
    parser.add_argument('--files', type=str,
                       help='Specific files to fix (comma-separated)')
    parser.add_argument('--all', action='store_true',
                       help='Fix all Python files with Unicode issues')
    parser.add_argument('--verify', action='store_true',
                       help='Only verify fixes, don\'t apply them')
