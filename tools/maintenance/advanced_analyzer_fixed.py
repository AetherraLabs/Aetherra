#!/usr/bin/env python3
"""
Advanced Project Intelligence System - Fixed Version
...
class AdvancedProjectAnalyzer:
    ...
    def extract_decorator_name(self, decorator_node):
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.Import):
                    ...
                    analysis['classes'].append({
                        ...
                        'decorators': [self.extract_decorator_name(d) for d in node.decorator_list],
    ...
    def analyze_markdown_file(self, filepath, content):
        ...
        analysis['images'] = [{'alt': img[0], 'url': img[1]} for img in images]

        # Extract code blocks
        code_pattern = r'```(\w+)?\n(.*?)\n```'
        code_blocks = re.findall(code_pattern, content, re.DOTALL)
        analysis['code_blocks'] = [{'language': code[0], 'code': code[1][:100]} for code in code_blocks]
    ...
    def analyze_config_file(self, filepath, content):
        ...
        analysis = {
            ...
            'keys': [],
            'structure_depth': 0,
            'is_package_config': filepath.name in ['package.json', 'pyproject.toml', 'requirements.txt']
        }
    ...
    def infer_file_purpose(self, filepath, content, analysis):
        ...
        # Configuration files
        if name in ['config.py', 'settings.py', 'config.json', '.env', 'pyproject.toml']:
    ...
    def calculate_dict_depth(self, data):
        """Calculate the depth of a nested dictionary"""
        if isinstance(data, dict) and data:
            return 1 + max(self.calculate_dict_depth(v) for v in data.values())
        elif isinstance(data, list) and data:
            return max(self.calculate_dict_depth(item) for item in data if isinstance(item, (dict, list)))
        return 0
        ...
        try:
            ...
            for item in directory.iterdir():
                if item.is_file():
                    ...
                    # Identify main files
                    if item.name.lower() in ['main.py', 'index.js', 'app.py', '__init__.py']:
            ...
            test_files = sum(1 for f in directory.glob('**/test_*.py')) + sum(1 for f in directory.glob('**/*_test.py'))
            doc_files = sum(1 for f in directory.glob('**/*.md')) + sum(1 for f in directory.glob('**/README*'))

            if analysis['total_files'] > 0:
                analysis['test_coverage'] = (test_files / analysis['total_files']) * 100
                analysis['documentation_coverage'] = (doc_files / analysis['total_files']) * 100

            # Quality indicators
            analysis['quality_indicators'] = {
                'has_readme': any(f.name.lower().startswith('readme') for f in directory.glob('*') if f.is_file()),
                'has_tests': test_files > 0,
                'has_documentation': doc_files > 0,
                'organized_structure': len(analysis['subdirectories']) > 0,
                'complexity_per_file': analysis['complexity_score'] / max(analysis['total_files'], 1)
            }
    ...
    def run_comprehensive_analysis(self):
        ...
        for root, dirs, files in os.walk(self.project_root):
            ...
            for filename in files:
                ...
                if total_files % 50 == 0:
                    print(f"   📊 Analyzed {total_files} files and {total_dirs} directories...")
