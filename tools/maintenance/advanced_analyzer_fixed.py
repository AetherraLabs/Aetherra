...
class AdvancedProjectAnalyzer:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.Import):
                    ...
                    func_info = {
                        ...
                        "decorators": [
                            self.extract_decorator_name(d) for d in node.decorator_list
                        ],
                        "docstring": ast.get_docstring(node),
                    }
    ...
    def analyze_markdown_file(self, filepath, content):
        ...
        analysis["code_blocks"] = [
            {"language": code[0], "code": code[1][:100]} for code in code_blocks
        ]
    ...
    def calculate_dict_depth(self, data):
        ...
        if isinstance(data, list) and data:
            nested_items = [item for item in data if isinstance(item, dict | list)]
            if not nested_items:
                return 0
            return max(
