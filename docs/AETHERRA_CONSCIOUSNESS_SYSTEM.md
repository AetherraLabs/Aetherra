...
- `ContinuityMemory.record` and direct `save` calls declare `consciousness.continuity_save` before appending continuity snapshots, creating persistence directories, writing temp files, or replacing the continuity JSON file.
...
- `ConsciousnessCore._reflect_micro` and `_reflect_macro` declare `consciousness.micro_reflection_update` and `consciousness.macro_reflection_update` before narrative-thread mutation, qualia-learning decay, or optional QFAC reflection persistence. Denied decisions skip those reflection side effects.
