...
from .personal_history import (
    MemoryImportance,
    MemoryType,
    PersonalHistorySystem,
    get_personal_history,
)
from .self_model import PersonalityAspect, SelfModelSystem
...
class IdentityAgent:
    ...
    def __init__(self):
        # Initialize subsystems (using provided factory/accessors)
        self.beliefs_system = CoreBeliefsSystem()
        self.history_system = get_personal_history()
        self.self_model_system = SelfModelSystem()
        self.logger = logging.getLogger(__name__)
        self.identity_coherence_score = 0.0  # type: float
    ...
    def get_identity_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "fundamental_beliefs": [
                belief.name
                for belief in self.beliefs_system.beliefs.values()
        ...
        for option in options:
            ...
            if relevant_memories:
                ...
                positive_outcomes = [
                    ...
                    if m.importance.value in ["significant", "defining"]
                ]
    ...
    def reflect_on_experience(
        self, experience_description: str, outcome: str, lessons_learned: List[str]
    ) -> Dict[str, Any]:
        """
        ...
        for belief_name, belief in self.beliefs_system.beliefs.items():
            if any(
                word in experience_description.lower()
                for word in belief.description.lower().split()[:5]
            ):
        ...
        reflection_summary = {
            ...
            "capabilities_updated": len(
                [lesson for lesson in lessons_learned if "better at" in lesson.lower()]
            ),
    ...
    def _check_identity_coherence(self) -> float:
        ...
        memory_belief_coherence = min(
            belief_mentioned_in_memories / max(len(defining_memories), 1), 1.0
        )
    ...
    def export_complete_identity(self) -> Dict[str, Any]:
        ...
        return {
            "identity_agent": {
                ...
                "last_coherence_check": self._last_coherence_check.isoformat()
                if self._last_coherence_check
                else None,
            },
...
_lyrixa_identity_agent: Optional[IdentityAgent] = None


def get_identity_agent() -> IdentityAgent:
    """Get the global identity agent instance (singleton pattern)"""
    global _lyrixa_identity_agent
    if _lyrixa_identity_agent is None:
        _lyrixa_identity_agent = IdentityAgent()
    return _lyrixa_identity_agent
...
if __name__ == "__main__":
    ...
    decision = agent.make_identity_based_decision(
        ...
        [
            "Admit uncertainty and offer to help research",
            "Pretend to know and give a confident answer",
