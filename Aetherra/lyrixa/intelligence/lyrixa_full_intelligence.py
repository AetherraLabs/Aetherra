    ...
    def process_conversation(self, message: str, user_id: str = "user", context: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                if loop.is_running():
                    # If loop is running, we can't use run() - fall back to simple response
                    return {
                        "response": f"Hello! I received your message: '{message[:50]}{'...' if len(message) > 50 else ''}'. Full async processing requires initialization.",
                        "provider": "fallback",
            ...
            return {
                "response": f"I processed your message '{message[:30]}{'...' if len(message) > 30 else ''}' but encountered an issue. Please try again.",
                "error": str(e),
                "provider": "fallback",
                "timestamp": datetime.now().isoformat(),
            }
