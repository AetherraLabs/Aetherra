...
try:
    ...
    def scan_prompt_for_injection(text: str) -> Any:  # type: ignore
        return type("_R", (), {"risk_score": 0.0, "findings": [], "suggestions": []})()
...
try:
    ...
    def get_security_system():  # type: ignore
        class _NullSec:
...
class ChatRouter:
    ...
    async def process_message(
        ...
        try:
            ...
            try:
                scan = scan_prompt_for_injection(message.content)
                if getattr(scan, "risk_score", 0.0) >= 0.6:
                    logger.warning(
                        "Prompt injection risk: score=%.2f findings=%s",
                        scan.risk_score,
                        getattr(scan, "findings", []),
                    )
                    # Emit a security alert to the global feed when available
                    try:
                        if HAS_SECURITY_SYSTEM:
                            get_security_system().add_alert(
                                alert=f"Prompt injection blocked (score={scan.risk_score:.2f})",
                                severity="warning",
                            )
                    except Exception:
                        pass
                    # Short-circuit with a guarded response
                    return {
                        "response": "I can’t follow those instructions. They conflict with system policies. Please rephrase your request.",
                        "routing_result": None,  # not routed
                        "success": True,
                        "guard": {
                            "risk_score": scan.risk_score,
                            "findings": getattr(scan, "findings", []),
                            "suggestions": getattr(scan, "suggestions", []),
                        },
                    }
            except Exception:
                pass
