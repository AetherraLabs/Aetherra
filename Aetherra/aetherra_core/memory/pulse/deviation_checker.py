...
class DriftAlert:
    ...
    alert_id: str
    drift_type: str  # "concept_shift", "confidence_decay", "contradiction", "orphaned_memory"
...
class MemoryPulseMonitor:
    ...
    def run_pulse_check(
        ...
        active_concepts = len([c for c in concept_clusters if c.member_fragments])
    ...
    def _detect_contradictions(self, fragments: list[MemoryFragment]) -> list[tuple[str, str]]:
        ...
        tag_groups: dict[str, list[MemoryFragment]] = {}
        for fragment in fragments:
            for tag in fragment.symbolic_tags:
                tag_groups.setdefault(tag, []).append(fragment)

        # Look for contradictions within tag groups
        for _tag, tag_fragments in tag_groups.items():
    ...
    def get_active_alerts(self, severity_filter: str | None = None) -> list[DriftAlert]:
    ...
    def _get_last_maintenance_time(self) -> datetime | None:
