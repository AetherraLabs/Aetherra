...
class MemoryCompressionAnalyzer:
    ...
    async def analyze_memory_fragment(
        ...
        if not force_analysis:
            ...
            if existing_score:
                print(
                    f"   ✅ Using cached analysis (age: {time.time() - existing_score.compression_timestamp:.1f}s)"
                )
    ...
    async def get_compression_recommendations(
        ...
        for fragment in memory_fragments:
            ...
            fidelity_counts[score.fidelity_level.value] = (
                fidelity_counts.get(score.fidelity_level.value, 0) + 1
            )
    ...
    async def _select_optimal_schema(
        self, memory_type: str, score: CompressionScore
    ) -> CompressionSchema:
    ...
    async def _store_analysis_results(
        self, score: CompressionScore, memory_type: str, schema_used: str
    ):
        ...
        """,
            (
                ...
                memory_type,
                score.entropy,
                score.structure_depth,
                score.recursive_density,
                score.fidelity_level.value,
                score.compression_ratio,
                score.access_frequency,
                score.temporal_decay,
                score.original_size,
                score.compressed_size,
                score.reconstruction_quality,
                score.semantic_preservation,
                score.compression_timestamp,
                schema_used,
            ),
        )
    ...
    async def _update_performance_tracking(
        self, score: CompressionScore, memory_type: str, schema: CompressionSchema
    ):
        ...
        """,
            (
                performance.memory_type,
                performance.compression_time,
                performance.decompression_time,
                performance.original_size,
                performance.compressed_size,
                performance.compression_ratio,
                performance.fidelity_achieved.value,
                performance.access_frequency,
    ...
    def _get_recommended_action(self, score: CompressionScore) -> str:
        ...
        elif score.compression_ratio > 10.0 and score.fidelity_level in [
            FidelityLevel.LOSSLESS,
            FidelityLevel.LOSSY_SAFE,
        ]:
    ...
    def _calculate_compression_priority(self, score: CompressionScore) -> float:
        ...
        access_factor = 1.0 - min(
            score.access_frequency / 5.0, 1.0
        )  # Less accessed = higher priority
    ...
    async def _generate_global_recommendations(
        ...
        if total_original > 0:
            ...
            if overall_ratio > 5.0:
                recommendations.append(
                    "Excellent compression potential detected - proceed with aggressive compression"
                )
        ...
        if degraded_count > len(fragments) * 0.1:
            recommendations.append(
                f"Warning: {degraded_count} fragments may lose significant quality during compression"
            )
    ...
    async def _identify_performance_issues(
        ...
        for memory_type, metrics in performance_by_type.items():
            if metrics["avg_compression_ratio"] < 2.0:
                issues.append(
                    f"Poor compression ratio for {memory_type}: {metrics['avg_compression_ratio']:.1f}x"
                )

            if metrics["avg_compression_time"] > 1.0:
                issues.append(
                    f"Slow compression for {memory_type}: {metrics['avg_compression_time']:.3f}s"
                )

        return issues

    async def _generate_optimization_suggestions(
        self, performance_by_type: Dict[str, Any], issues: List[str]
    ) -> List[str]:
        """Generate optimization suggestions"""
        suggestions = []
...
async def demo_compression_analyzer():
    ...
    for fragment in test_fragments:
        ...
        print(
            f"   ✅ {fragment['id']}: {score.fidelity_level.value} | {score.compression_ratio:.1f}x compression"
        )
    ...
    print(
        f"   🎯 Overall compression ratio: {recommendations['summary']['estimated_compression_ratio']:.1f}x"
    )
