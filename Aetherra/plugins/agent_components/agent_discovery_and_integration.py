...
class AetherraComponentDiscovery:
        ...
        for root, dirs, files in os.walk(self.project_root):
            for file in files:
                if file.endswith(".py") and any(
                    pattern in file.lower() for pattern in other_patterns
                ):
                    file_path = Path(root) / file
                    if self._is_in_aetherra_path(file_path):
                        other.extend(
                            self._scan_file_for_classes(
                                file_path, "other_components", other_patterns
                            )
                        )

        return other

    def _scan_directory_for_classes(
        self, directory: Path, component_type: str, keywords: List[str]
    ) -> List[ComponentInfo]:
    ...
    def _scan_file_for_classes(
        self, file_path: Path, component_type: str, keywords: List[str]
    ) -> List[ComponentInfo]:
            ...
            for component in components:
                try:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"[WARN] {component.name} available but not instantiated: {init_error}"
                        )
