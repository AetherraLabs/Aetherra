...
class ComprehensiveAgentDiscovery:
    ...
    def comprehensive_agent_scan(self):
        ...
        for file_path in all_py_files:
            try:
                self._analyze_file_for_agents(
                    file_path, agents_found, agent_patterns, agent_class_patterns
                )
            except Exception as e:
                print(f"[WARN]  Could not analyze {file_path.name}: {e}")
    ...
    def _analyze_file_for_agents(
        self, file_path, agents_found, agent_patterns, agent_class_patterns
    ):
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.ClassDef):
                    ...
                    self._analyze_function_node(
                        node, file_path, rel_path, agents_found, agent_patterns, content
                    )

            # Look for specific AI/agent patterns in content
            self._analyze_content_patterns(
                content, content_lower, file_path, rel_path, agents_found
            )
    ...
    def _analyze_class_node(
        self, node, file_path, rel_path, agents_found, agent_class_patterns, content
    ):
        ...
        for base in node.bases:
            if isinstance(base, ast.Name) and any(
                pattern in base.id for pattern in agent_class_patterns
            ):
                agents_found["class_based_agents"].append(
                    {
                        "file": str(rel_path),
    ...
    def _analyze_function_node(
        self, node, file_path, rel_path, agents_found, agent_patterns, content
    ):
        ...
        for pattern in ai_patterns:
            if re.search(pattern, content_lower):
                agents_found["ai_handlers"].append(
                    {"file": str(rel_path), "pattern": pattern, "type": "ai_usage"}
                )
                break
...
def main():
    ...
    # Save results
    discovery_report = discovery.save_discovery_results(
        agents_found, priority_agents, migration_plan
    )

    print("\n🎉 AGENT DISCOVERY COMPLETE!")
    print("✅ Scanned entire codebase for agents")
    print(f"✅ Found {discovery_report['summary']['total_agents']} potential agents")
    print("✅ Prioritized agents into 4 categories")
    print("✅ Created 6-phase migration plan")
    print("✅ Saved comprehensive discovery report")
