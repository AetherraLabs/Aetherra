...
def test_qfac_retrieval_parity_per_k_counters_schema_contains_help_type_and_lines():
    ...
    for k in (1, 3, 5, 10):
        ...
        assert re.search(
            rf"^# HELP aetherra_qfac_retrieval_parity_top{k}_match_total ",
            text,
            flags=re.MULTILINE,
        )
        assert re.search(
            rf"^# TYPE aetherra_qfac_retrieval_parity_top{k}_match_total counter$",
            text,
            flags=re.MULTILINE,
        )
        # Ensure metric line exists
        assert re.search(
            rf"^aetherra_qfac_retrieval_parity_top{k}_match_total ",
            text,
            flags=re.MULTILINE,
        )
