...
class AutoRoundQuantizer(HfQuantizer):
    ...
    def update_torch_dtype(self, torch_dtype: "torch.dtype") -> "torch.dtype":
        if torch_dtype is None:
            ...
            logger.info(
                "Loading the model in `torch.bfloat16`. To overwrite it, set `torch_dtype` manually."
            )
        return torch_dtype
    ...
    def _process_model_before_weight_loading(self, model: "PreTrainedModel", **kwargs):
        if model.__class__.main_input_name != "input_ids":
            logger.warning(
                "AutoRound offers only limited support for models that are not strictly text-based."
            )
        from auto_round.inference.convert_model import (
            convert_hf_model,
            infer_target_device,
        )
