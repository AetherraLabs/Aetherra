...
class TFNoRepeatNGramLogitsProcessor(TFLogitsProcessor):
    ...
    def calc_banned_ngram_tokens(self, input_ids, num_hypos, cur_len):
        ...
        for idx in range(num_hypos):
            ...
            for ngram in zip(
                *[gen_tokens[i:] for i in range(self.ngram_size)], strict=False
            ):
            ...
            scores = tf.cond(
                ...
                lambda: tf.tensor_scatter_nd_update(
                    ...
                    updates=[
                        -float("inf")
                        for _ in range(
                            scores.shape[0] * len(self.begin_suppress_tokens)
                        )
                    ],
                ),
                lambda: scores,
...
class TFForceTokensLogitsProcessor(TFLogitsProcessor):
    ...
    def __call__(
        ...
        def _force_token(generation_idx):
            ...
            new_scores = tf.zeros_like(scores, dtype=scores.dtype) + tf.constant(
                [scores.dtype.min]
            )
            indices = tf.stack(
