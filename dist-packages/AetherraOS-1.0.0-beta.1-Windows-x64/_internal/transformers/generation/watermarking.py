...
class WatermarkDetectorOutput:
    ...
    num_tokens_scored: np.array | None = None
    num_green_tokens: np.array | None = None
    green_fraction: np.array | None = None
    z_score: np.array | None = None
    p_value: np.array | None = None
...
class BayesianDetectorConfig(PretrainedConfig):
    ...
    def __init__(
        self, watermarking_depth: int | None = None, base_rate: float = 0.5, **kwargs
    ):
