...
class AssistedCandidateGenerator(CandidateGenerator):
    ...
    def __init__(
        ...
        if (
            "logits_to_keep" in assistant_kwargs
...
class _PruneReindexingLMHead(nn.Module):
    ...
    def __init__(self, original_lm_head, assistant_overlap_token_ids):
        super().__init__()
        self.pruned_lm_head = prune_linear_layer(
            original_lm_head, assistant_overlap_token_ids
        ).to(original_lm_head.weight.dtype)
...
class _MapInputEmbedding(nn.Module):
    ...
    def forward(self, input_ids: torch.LongTensor) -> torch.FloatTensor:
        ...
        if self.map:
            ...
            my_input_ids = (
                self.assistant_overlap_token_ids[input_ids[0, -1]]
                .unsqueeze(0)
                .unsqueeze(0)
            )
        else:
...
class AssistantToTargetTranslator:
    ...
    def __init__(
        ...
        self.assistant_prune_lm_head = (
            assistant_prune_lm_head and assistant_model is not None
        )
        if len(self._suppress_input_ids) > 0:
            ...
            if self.assistant_prune_lm_head:
                ...
                pruned_lm_head = _PruneReindexingLMHead(
                ...
                map_input_embeddings = _MapInputEmbedding(
                    original_input_embeddings, self.assistant_overlap_token_ids
                )
    ...
    def _get_assistant_to_target_input_ids(self):
        ...
        return assistant_to_target_input_ids.to(
    ...
    def get_target_logits(
        ...
        if self.assistant_prune_lm_head:
            ...
            target_logits[..., target_logits_supported_indices] = (
                valid_assistant_logits[..., assistant_indices_mask]
            )
