...
class TrainingSummary:
    ...
    def create_model_index(self, metric_mapping):
        ...
        dataset_mapping = dict(zip(dataset_tags, dataset_names, strict=False))
        dataset_arg_mapping = dict(zip(dataset_tags, dataset_args, strict=False))
        dataset_metadata_mapping = dict(
            zip(dataset_tags, dataset_metadata, strict=False)
        )
...
def _regular_table_line(values, col_widths):
    values_with_space = [
        f"| {v}" + " " * (w - len(v) + 1)
        for v, w in zip(values, col_widths, strict=False)
    ]
