...
def _tf_collate_batch(examples, tokenizer, pad_to_multiple_of: int | None = None):
    import tensorflow as tf
...
class DataCollatorForMultipleChoice(DataCollatorMixin):
    ...
    def torch_call(
    ...
    ):  # Refactored implementation from the docs.
        import torch
        ...
        flat_examples = sum(
            (
                [{k: v[i] for k, v in example.items()} for i in range(num_choices)]
                for example in examples
            ),
            start=[],
        )
    ...
    def tf_call(self, features):  # Implementation taken from the docs.
        ...
        flattened_features = sum(
            flattened_features, []
        )  # Sometimes written as list(chain(*flattened_features))
    ...
    def __call__(self, features, return_tensors=None):
        ...
        if labels is not None:
            if no_padding:
                ...
                max_label_length = (
                    max(len(l) for l in labels) if not max_padding else self.max_length
                )
                ...
                if isinstance(features[0][label_name], list):
                    batch["labels"] = [
                        ...
                        if padding_side == "right"
                        else [self.label_pad_token_id] * (max_label_length - len(label))
                        + label
                        for label in labels
                    ]
                else:
...
class DataCollatorForLanguageModeling(DataCollatorMixin):
    ...
    def tf_mask_tokens(
        ...
        masked_indices = (
            self.tf_bernoulli(input_shape, self.mlm_probability, self.generator)
            & ~special_tokens_mask
        )
        ...
        indices_replaced = (
            self.tf_bernoulli(input_shape, self.mask_replace_prob, self.generator)
            & masked_indices
        )
        ...
        if self.generator:
            random_words = self.generator.uniform(
    ...
    def torch_mask_tokens(
        self, inputs: Any, special_tokens_mask: Any | None = None
    ) -> tuple[Any, Any]:
        """
        Prepare masked tokens inputs/labels for masked language modeling: 80% MASK, 10% random, 10% original.
        """
    ...
    def numpy_mask_tokens(
        ...
        if self.generator:
            masked_indices = self.generator.binomial(
        ...
        if self.generator:
            indices_replaced = (
                self.generator.binomial(
                    1, self.mask_replace_prob, size=labels.shape
                ).astype(bool)
                & masked_indices
            )
        else:
            indices_replaced = (
                np.random.binomial(1, self.mask_replace_prob, size=labels.shape).astype(
                    bool
                )
                & masked_indices
            )
        ...
        if self.generator:
            indices_random = (
                self.generator.binomial(
                    1, random_replace_prob_scaled, size=labels.shape
                ).astype(bool)
                & masked_indices
                & ~indices_replaced
            )
        ...
        if self.return_tensors == "tf":
            ...
            indices = (
                tf.random.experimental.stateless_shuffle(
                    tf.range(len(cand_indexes)), seed=seed
                )
                .numpy()
                .tolist()
            )
...
class DataCollatorWithFlattening(DefaultDataCollator):
    ...
    def __call__(self, features, return_tensors=None, separator_id=None):
        ...
        if return_tensors == "pt":
            ...
            raise ValueError(
                f'return_tensors must be one of ("pt", "np"), {return_tensors=} not suported'
            )
        ...
        for k, v in batch.items():
            ...
            if k not in self._py_int_keys:
                batch[k] = data_cls(
                    v, dtype=dtype_64 if k in self._int_64_keys else dtype_32
                )
