...
def load_audio(
    ...
    if isinstance(audio, str):
        ...
        if audio.startswith("http://") or audio.startswith("https://"):
            audio = librosa.load(
                BytesIO(requests.get(audio, timeout=timeout).content), sr=sampling_rate
            )[0]
...
def chroma_filter_bank(
    ...
    start_at_c_chroma: bool | None = True,
):
    """
...
def mel_filter_bank(
    ...
    if min_frequency > max_frequency:
        raise ValueError(
            f"Require min_frequency: {min_frequency} <= max_frequency: {max_frequency}"
        )
