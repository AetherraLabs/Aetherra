    ...
    for (
        ...
        if capturable:
            ...
            rect = [
                ...
                for n, rho_t in zip(num, rho_t_list, strict=False)
            ]
            del num
            del rho_t_list
            ...
            unrect_step_size = [
                (lr * rect / bc) * -1
                for rect, bc in zip(unrectified, bias_correction1, strict=False)
            ]
            bias_correction2 = [
                ((1 - beta2 ** _get_value(step)) ** 0.5) * (lr * rect / bc) * -1
                for step, rect, bc in zip(
                    grouped_state_steps, rect, bias_correction1, strict=False
                )
            ]
