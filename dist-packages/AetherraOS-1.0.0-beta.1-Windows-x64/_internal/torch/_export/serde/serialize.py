...
def _get_schema_from_target(target):
    ...
    if type(target) in _serialization_registry:
...
class GraphModuleSerializer(metaclass=Final):
    ...
    def serialize_operator(self, target) -> str:
        if isinstance(target, str):
            return target
        if target.__module__.startswith("torch._ops"):
        ...
        # TODO(zhxchen17) Don't catch all here.
    ...
    def serialize_sym_op_inputs(self, op, args) -> list[NamedArgument]:
        ...
        serialized_args = []
        for args_name, arg in zip(args_names, args, strict=False):
    ...
    def serialize_input(self, arg, arg_type: Any | None = None) -> Argument:
        ...
        if isinstance(arg, torch.fx.Node):
            ...
            if arg.name in self.duplicate_getitem_nodes:
        ...
        if isinstance(arg, inductor_tensor_buffers):
        ...
        if isinstance(arg, inductor_ir.TorchBindObject):
        ...
        if isinstance(arg, (list, tuple)):
            if len(arg) == 0:
                if arg_type is not None:
                    ...
                    if isinstance(elem_type, torch.IntType):
                        return Argument.create(as_ints=[])
                    if isinstance(elem_type, torch.FloatType):
                        return Argument.create(as_floats=[])
                    if isinstance(elem_type, torch.StringType):
                        return Argument.create(as_strings=[])
                    if isinstance(elem_type, torch.TensorType):
            ...
            if all(type(a) is float for a in arg):
                return Argument.create(as_floats=list(arg))
            if all(type(a) is str for a in arg):
                return Argument.create(as_strings=list(arg))
            if all(self.is_inductor_sym_int_arg(a) for a in arg):
                # This is a special branch for handling SymInt args in inductor's
                # ExternalFallbackNode.
            ...
            if all(isinstance(a, torch.SymFloat) for a in arg):
                return Argument.create(
                    as_sym_floats=[SymFloatArgument.create(as_name=str(a)) for a in arg]
                )
            if all(self.is_sym_int_arg(a) for a in arg):
                # list of sym_ints
                values = []
                for a in arg:
                    if isinstance(a, torch.fx.Node):
                        ...
                        values.append(SymIntArgument.create(as_int=a))
                return Argument.create(as_sym_ints=values)
            if all(self.is_sym_float_arg(a) for a in arg):
                # list of sym_float
                values = []
                for a in arg:
                    if isinstance(a, torch.fx.Node):
            ...
            if all(self.is_sym_bool_arg(a) for a in arg):
                # list of sym_bools
                values = []
                for a in arg:
                    if isinstance(a, torch.fx.Node):
            ...
            if all(isinstance(a, (torch.fx.Node, type(None))) for a in arg):
            ...
            if all(isinstance(a, inductor_tensor_buffers) for a in arg):
                # list of inductor buffers
                return Argument.create(
                    as_tensors=[TensorArgument(name=a.get_name()) for a in arg],
                )
            if all(isinstance(a, (*inductor_tensor_buffers, type(None))) for a in arg):
                # list of inductor buffers as optional tensors
                def serialize_optional_tensor_args(a):
                    if a is None:
                        return OptionalTensorArgument.create(as_none=True)
                    if isinstance(a, inductor_tensor_buffers):
                        return OptionalTensorArgument.create(
        ...
        if isinstance(arg, torch._C.ScriptObject):
            if not (
        ...
        if isinstance(arg, (torch._ops.OpOverload, torch._ops.HigherOrderOperator)):
    ...
    def serialize_input_spec(self, spec: ep.InputSpec) -> InputSpec:
        ...
        if spec.kind == ep.InputKind.BUFFER:
            assert spec.target is not None
            assert isinstance(spec.arg, ep.TensorArgument)
        ...
        if spec.kind == ep.InputKind.CONSTANT_TENSOR:
            assert spec.target is not None
            assert isinstance(spec.arg, ep.TensorArgument)
            return InputSpec.create(
        ...
        if spec.kind == ep.InputKind.CUSTOM_OBJ:
            assert spec.target is not None
        ...
        if spec.kind == ep.InputKind.TOKEN:
    ...
    def serialize_output_spec(self, spec: ep.OutputSpec) -> OutputSpec:
        ...
        if spec.kind == ep.OutputKind.LOSS_OUTPUT:
            assert isinstance(spec.arg, ep.TensorArgument)
            return OutputSpec.create(
                loss_output=LossOutputSpec(arg=TensorArgument(name=spec.arg.name))
            )
        if spec.kind == ep.OutputKind.BUFFER_MUTATION:
            assert spec.target is not None
            assert isinstance(spec.arg, ep.TensorArgument)
            return OutputSpec.create(
        ...
        if spec.kind == ep.OutputKind.GRADIENT_TO_PARAMETER:
            assert spec.target is not None
            assert isinstance(spec.arg, ep.TensorArgument)
            return OutputSpec.create(
        ...
        if spec.kind == ep.OutputKind.GRADIENT_TO_USER_INPUT:
            assert spec.target is not None
            assert isinstance(spec.arg, ep.TensorArgument)
            return OutputSpec.create(
        ...
        if spec.kind == ep.OutputKind.USER_INPUT_MUTATION:
            assert spec.target is not None
            assert isinstance(spec.arg, ep.TensorArgument)
            return OutputSpec.create(
        ...
        if spec.kind == ep.OutputKind.TOKEN:
            assert isinstance(spec.arg, ep.TokenArgument)
            return OutputSpec.create(
    ...
    def serialize_outputs(self, node: torch.fx.Node) -> list[Argument]:
        ...
        output_arguments = []
        for idx, (meta, return_schema) in enumerate(
            zip(meta_val, returns, strict=False)
        ):
            if meta is None:
                assert isinstance(
...
class GraphModuleDeserializer(metaclass=Final):
    ...
    def deserialize_output_spec(self, o: OutputSpec) -> ep.OutputSpec:
        ...
        if o.type == "gradient_to_parameter":
            return ep.OutputSpec(
        ...
        if o.type == "gradient_to_user_input":
            return ep.OutputSpec(
    ...
    def deserialize_input(self, inp: Argument) -> Any:
        ...
        if isinstance(value, list):
            ...
            if typ_ == "as_tensors":
                ...
                return result
            if typ_ in ("as_ints", "as_floats", "as_bools", "as_strings"):
                # convert from serialized.python.types.List to python list
                return list(value)
            if typ_ in ("as_sym_ints", "as_sym_bools", "as_sym_floats"):
                return [self.deserialize_sym_argument(arg) for arg in value]
            if typ_ == "as_optional_tensors":
    ...
    def deserialize_constant_input(self, inp: ConstantValue) -> Any:
        ...
        if inp.type == "as_float":
            return float(inp.as_float)
        if inp.type == "as_string":
    ...
    def deserialize_outputs(self, serialized_node: Node, fx_node: torch.fx.Node):
        ...
        if (
            ...
            return None
        if len(serialized_node.outputs) == 1 and isinstance(
            serialized_node.outputs[0].value,
            (SymIntArgument, SymBoolArgument, SymFloatArgument),
        ):
            self.sync_fx_node(serialized_node.outputs[0].value.as_name, fx_node)
    ...
    def generate_getitem(
        ...
        if isinstance(arg, TensorArgument):
            name = arg.name
        elif isinstance(arg, SymIntArgument) or isinstance(arg, SymFloatArgument):
            name = arg.as_name
        else:
...
def _canonicalize_graph(
    ...
    def _get_argument(a: Argument):
        ...
        if (
            ...
            or a.type == "as_floats"
            or a.type == "as_string"
            or a.type == "as_strings"
        ):
            return None
        if a.type == "as_sym_int":
            return a.as_sym_int
        if a.type == "as_sym_ints":
            return a.as_sym_ints
        if a.type == "as_sym_float":
            return a.as_sym_float
        if a.type == "as_sym_floats":
            return a.as_sym_floats
        if (
            a.type == "as_scalar_type"
            or a.type == "as_memory_format"
            or a.type == "as_layout"
            or a.type == "as_device"
            or a.type == "as_bool"
            or a.type == "as_bools"
            ...
            return None
        if a.type == "as_sym_bool":
            return a.as_sym_bool
        if a.type == "as_sym_bools":
            return a.as_sym_bools
        if a.type == "as_graph":
            return None
        if a.type == "as_optional_tensors":
            return a.as_optional_tensors
        if a.type == "as_custom_obj":
            return a.as_custom_obj
        if a.type == "as_operator":
            return None
    ...
    def sort_nodes(nodes):
        ...
        def get_name(a) -> str | None:
            ...
            if isinstance(a, TensorArgument):
                return a.name
            if isinstance(a, (SymIntArgument, SymBoolArgument, SymFloatArgument)):
                if a.type == "as_name":
                    return a.as_name
                if a.type in ("as_int", "as_bool", "as_float"):
                    return None
                raise AssertionError(f"Unknown argument type: {a}")
            if isinstance(a, OptionalTensorArgument):
                if a.type == "as_tensor":
                    return a.as_tensor.name
                if a.type == "as_none":
                    return None
                raise AssertionError(f"Unknown optional tensor type: {a}")
            if isinstance(a, CustomObjArgument):
                return a.name
            raise AssertionError(f"Unknown argument type: {a}")
    ...
    def replace_use(a):
        ...
        elif isinstance(a, (SymIntArgument, SymFloatArgument)) or isinstance(
            a, SymBoolArgument
        ):
            if a.type == "as_name":
...
def canonicalize(
    ...
    def rank_output(out) -> tuple[int, str | None, int]:
        ...
        if spec.type == "user_output" or spec.type == "loss_output":
            return 3, None, idx
        if spec.type == "buffer_mutation":
            return 1, spec.buffer_mutation.buffer_name, idx
        if spec.type == "gradient_to_parameter":
            return 4, spec.gradient_to_parameter.parameter_name, idx
        if spec.type == "gradient_to_user_input":
            return 5, None, idx
        if spec.type == "user_input_mutation":
            return 2, None, idx
        if spec.type == "token":
            return 0, None, idx
    ...
    sorted_ins = sorted(
        enumerate(zip(graph.inputs, signature.input_specs, strict=False)),
        key=rank_input,
    )

    if len(sorted_ins) > 0:
        sorted_inputs, input_specs = zip(*(i for idx, i in sorted_ins), strict=False)  # type: ignore[assignment]
    else:
        sorted_inputs = ()
        input_specs = ()

    sorted_outs = sorted(
        enumerate(zip(graph.outputs, signature.output_specs, strict=False)),
        key=rank_output,
    )
    sorted_outputs, output_specs = zip(*(i for idx, i in sorted_outs), strict=False)  # type: ignore[assignment]
