...
from typing import Any, NoReturn, Protocol, Self, TypeVar, Union
        ...
        with context:
            ...
            if should_propagate_eager_input_vals(self.nodes):
                ...
                for old_node, new_node in zip(
                    graph_with_eager_vals.graph.nodes,
                    replacement.graph.nodes,
                    strict=False,
                ):
...
class _TargetExpr(PatternExpr):
    ...
    def fns_repr(self) -> str:
        ...
        if len(self.fns) > 1:
            return f"[{first_repr}, ...]"
        if self.fns[0] is getattr(torch, first_repr, None):
            return f"torch.{first_repr}"
        if self.fns[0] is getattr(operator, first_repr, None):
            return f"operator.{first_repr}"
        if isinstance(self.fns[0], torch._ops.OpOverload):
    ...
    def pattern_eq(self, other: Any) -> bool:
        ...
        return (
            ...
            and all(
                ...
                for a, b in zip(
                    self.flat_args_kwargs[0], other.flat_args_kwargs[0], strict=False
                )
            )
        )
...
class MultiOutputPattern(PatternExpr):
    ...
    def pattern_eq(self, other: Any) -> bool:
        ...
        return (
            ...
            and all(
                a.pattern_eq(b) if isinstance(a, PatternExpr) else a == b
                for a, b in zip(self.outputs, other.outputs, strict=False)
            )
        )
    ...
    def replace_with_graph(
        ...
        with graph.inserting_before(last_node):
            ...
            if len(output_nodes) == len(replacement):
                for old, new in zip(output_nodes, replacement, strict=False):
                    replace(old, new)
            else:
    ...
    class Converter(torch.fx.Interpreter):
        ...
        def run_node(self, n: torch.fx.Node) -> Any:
            ...
            if n.op == "output" and isinstance(rv, tuple):
                ...
                for r, arg in zip(rv, args, strict=False):
