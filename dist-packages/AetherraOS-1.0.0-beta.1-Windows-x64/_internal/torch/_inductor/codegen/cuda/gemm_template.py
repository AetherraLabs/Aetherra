...
class CUTLASSGemmTemplate(CUTLASSTemplate, ABC):
    ...
    def cutlass_layout(torch_layout: ir.Layout) -> "cutlass_lib.LayoutType | None":  # type: ignore[name-defined]  # noqa: F821
        ...
        if V.graph.sizevars.statically_known_equals(torch_layout.stride[-2], 1):
    ...
    def render(  # type: ignore[override]
        ...
        op: "cutlass_gemm_op.GemmOperation" = None,  # type: ignore[name-defined]  # noqa: F821
        template_buffer_node: CUDATemplateBuffer | None = None,
        ...
        **kwargs,
        ...
        options.update(dict(zip(extra_names, extra_inputs, strict=False)))
    ...
    def test_call_statement(
        ...
        arguments = [
            ...
            for arg_type, arg_name in zip(arg_types, arg_names, strict=False)
        ]
...
class CUTLASS3xGemmTemplate(CUTLASSGemmTemplate):
    ...
    def _are_inputs_layout_compatible(self, layouts: list[Layout]) -> bool:
        ...
        M = A_size[-2]
        N = B_size[-1]
        if A_size[-1] != K and A_size[-1] != 1:
            return False
        if B_size[-2] != K and B_size[-1] != 1:
            return False
        ...
        if len(layouts) == 3:
            ...
            if len(C_size) > len(A_size):
                ...
                if C_size[len(A_size) - 2] != M and C_size[len(A_size) - 2] != 1:
                    return False
                remaining_size = 1
                for i in range(len(A_size) - 1, len(C_size)):
                    remaining_size *= C_size[i]
                if remaining_size != N and remaining_size != 1:
                    return False
                return True
            assert len(C_size) == len(A_size)
            if C_size[-2] != M and C_size[-2] != 1:
                return False
            if C_size[-1] != N and C_size[-1] != 1:
                return False
        return True
...
class CUTLASS2xGemmTemplate(CUTLASSGemmTemplate):
    ...
    def _are_inputs_layout_compatible(self, layouts: list[Layout]) -> bool:
        ...
        return (A_size[1] == K or 2 * A_size[1] == K) and B_size[0] == K
