...
class CUDAGraphNode:
    ...
    def reconstruct_outputs(self) -> OutputType:
        ...
        for i, (storage_info, metadata) in enumerate(
            zip(self.output_storage_alias, self.outputs_metadata, strict=False)
        ):
    ...
    def prepare_storages_for_construction(
        ...
        for output_storage_alias, metadata in zip(
            self.output_storage_alias, self.outputs_metadata, strict=False
        ):
    ...
    def _add_first_outputs(
        ...
        for out, static_output_tensor in zip(
            outputs, self.static_output_tensors, strict=False
        ):
    ...
    def _get_different_indices(
        ...
        for i, (outputs1, outputs2) in enumerate(zip(prev, curr, strict=False)):
            assert len(outputs1) == len(outputs2)
            for j, (output1, output2) in enumerate(
                zip(outputs1, outputs2, strict=False)
            ):
                if output1 != output2:
...
class CUDAGraphTreeManager:
    ...
    def dealloc_current_path_weakrefs(self) -> None:
        ...
        for node in self.current_node._path_from_root:
            ...
            for t, stack_trace in zip(
                node.tensor_weakrefs, node.stack_traces, strict=False
            ):
            ...
            for storage_ref, stack_trace in zip(
                node.outputs_weakrefs, node.stack_traces, strict=False
            ):
                if not storage_ref:
                    continue
