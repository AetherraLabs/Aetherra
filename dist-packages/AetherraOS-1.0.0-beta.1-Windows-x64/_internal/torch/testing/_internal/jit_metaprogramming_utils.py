...
def create_input(
    ...
    def map_arg(arg):
        ...
        if isinstance(arg, tuple) and len(arg) == 0:
            ...
            return var
        if isinstance(arg, tuple) and not isinstance(arg[0], torch.Tensor):
            return conjugate(
                maybe_non_contig(torch.randn(*arg, dtype=dtype, device=device))
            ).requires_grad_(requires_grad)
        # double check casting
        if isinstance(arg, non_differentiable):
...
def try_get_nn_module_compiled_mod_and_inputs(*args, **kwargs):
    ...
    _args, mod = (
        f_args_variable,
        create_script_module(None, nn_module, constructor_args, *f_args_variable)(
            *f_args_variable
        ),
    )
