...
class parametrize(_TestParametrizer):
    ...
    def _formatted_str_repr(self, idx, name, value):
        ...
        if type(value).__name__ in {"OpInfo", "ModuleInfo"}:
    ...
    def _default_subtest_name(self, idx, values):
        return "_".join(
            [
                ...
                for a, v in zip(self.arg_names, values, strict=False)
            ]
        )
    ...
    def _parametrize_test(self, test, generic_cls, device_cls):
        if len(self.arg_names) == 0:
            ...
            for idx, values in enumerate(self.arg_values):
                ...
                param_kwargs = dict(zip(self.arg_names, values, strict=False))
...
def make_dynamo_test(fn: Callable[..., Any] | None = None) -> Callable[..., Any]:
    """
...
class RelaxedBooleanPair(BooleanPair):
    ...
    def _to_bool(self, bool_like, *, id):
        ...
        if type(bool_like) in self._supported_number_types:
...
class RelaxedNumberPair(NumberPair):
    ...
    def _to_number(self, number_like, *, id):
        if isinstance(number_like, (torch.Tensor, np.ndarray)):
            ...
            return number
        if isinstance(number_like, Enum):
...
class TestCase(expecttest.TestCase):
    ...
    def genSparseCompressedTensor(
        ...
        *,
        ...
        sparse_dim = 2
        assert all(size[d] > 0 for d in range(len(size))) or nnz == 0, (
            "invalid arguments"
        )
        ...
        sparse_tensors_it = map(list, zip(*sparse_tensors, strict=False))
    ...
    def genSparseTensor(self, size, sparse_dim, nnz, is_uncoalesced, device, dtype):
        ...
        assert all(size[d] > 0 for d in range(sparse_dim)) or nnz == 0, (
            "invalid arguments"
        )
    ...
    def assertRaisesRegex(self, expected_exception, expected_regex, *args, **kwargs):
        ...
        if (
            ...
            and self.device_type not in NATIVE_DEVICES
