...
class IntelligenceReportGenerator:
    ...
    def generate_complete_directory_overview(self):
        ...
        report.append(
            f"- **Total Project Size:** {summary.get('complexity_analysis', {}).get('total_project_size', 0):,} bytes"
        )
        report.append(
            f"- **Average File Size:** {summary.get('complexity_analysis', {}).get('average_file_size', 0):.1f} bytes"
        )
        report.append("")
        ...
        sorted_dirs = sorted(
            directories.items(), key=lambda x: (len(Path(x[0]).parts), x[0])
        )
    ...
    def _add_directory_section(self, report, dir_path, dir_data):
        ...
        if dir_data["file_types"]:
            file_types_str = ", ".join(
                [f"{ext}({count})" for ext, count in dir_data["file_types"].items()]
            )
        ...
        if dir_data["programming_languages"]:
            langs_str = ", ".join(
                [
                    f"{lang}({count})"
                    for lang, count in dir_data["programming_languages"].items()
                    if lang != "Other"
                ]
            )
            if langs_str:
        ...
        if dir_data["entry_points"]:
            report.append(
                f"{indent}**Entry Points:** {', '.join(dir_data['entry_points'])}"
            )
            report.append("")
        ...
        quality_items = []
        if quality.get("has_readme"):
            quality_items.append("✅ README")
        if quality.get("has_tests"):
            quality_items.append("✅ Tests")
        if quality.get("has_documentation"):
            quality_items.append("✅ Docs")
        if quality.get("organized_structure"):
            quality_items.append("✅ Organized")
    ...
    def _add_python_details(self, report, file_data):
        ...
        if file_data.get("classes"):
            ...
            for cls in file_data["classes"][:5]:  # Limit to first 5
                methods_str = (
                    f" ({len(cls['methods'])} methods)" if cls["methods"] else ""
                )
    ...
    def _add_markdown_details(self, report, file_data):
        ...
        if file_data.get("code_blocks"):
            languages = [
                block.get("language", "text") for block in file_data["code_blocks"]
            ]
            lang_count = Counter(languages)
            if lang_count:
                lang_str = ", ".join(
                    [f"{lang}({count})" for lang, count in lang_count.most_common(3)]
                )
    ...
    def _add_config_details(self, report, file_data):
        ...
        if file_data.get("package_info"):
            ...
            if pkg.get("name"):
                report.append(
                    f"**Package:** {pkg['name']} v{pkg.get('version', 'unknown')}"
                )
    ...
    def generate_intelligence_insights(self):
        ...
        if main_components:
            ...
            for comp in main_components[:10]:
                report.append(
                    f"- **{comp['name']}**: {comp['purpose']} ({comp['files']} files, complexity: {comp['complexity']})"
                )
            report.append("")
        ...
        test_coverage = (
            len([d for d in directories.values() if "test" in d["purpose"].lower()])
            / len(directories)
            * 100
        )
        doc_coverage = (
            len(
                [
                    ...
                    if d["quality_indicators"].get("has_documentation")
                ]
            )
            / len(directories)
            * 100
        ...
        organized_dirs = (
            len(
                [
                    d
                    for d in directories.values()
                    if d["quality_indicators"].get("organized_structure")
                ]
            )
            / len(directories)
            * 100
        ...
        report.append(
            f"**Test Coverage:** {test_coverage:.1f}% of directories have testing"
        )
        report.append(
            f"**Documentation:** {doc_coverage:.1f}% of directories have documentation"
        )
        report.append(
            f"**Organization:** {organized_dirs:.1f}% of directories are well-organized"
        )
        report.append("")
        ...
        if complexity:
            ...
            if avg_complexity > 10:
                report.append(
                    "⚠️ **High complexity detected** - Consider refactoring complex files"
                )
            elif avg_complexity > 5:
                report.append(
                    "⚡ **Moderate complexity** - Good balance of functionality"
                )
            else:
                report.append(
                    "✅ **Low complexity** - Well-structured, maintainable code"
                )
            report.append("")
        ...
        if test_coverage < 30:
            recommendations.append(
                "📝 **Increase test coverage** - Add more test files to improve code reliability"
            )

        if doc_coverage < 50:
            recommendations.append(
                "📚 **Improve documentation** - Add README files and documentation to more directories"
            )

        if avg_complexity > 15:
            recommendations.append(
                "🔄 **Refactor complex code** - Break down large files into smaller, more manageable modules"
            )
        ...
        if duplicates:
            recommendations.append(
                f"🔍 **Remove duplicates** - Found {len(duplicates)} groups of duplicate files"
            )

        if not recommendations:
            recommendations.append(
                "✅ **Project is well-structured** - Continue following current best practices"
            )
    ...
    def save_all_reports(self):
        ...
        print(
            "✅ Complete Directory Intelligence saved to COMPLETE_DIRECTORY_INTELLIGENCE.md"
        )
        ...
        print(
            "✅ Project Intelligence Insights saved to PROJECT_INTELLIGENCE_INSIGHTS.md"
        )
