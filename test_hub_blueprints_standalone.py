...
class TestAgentsBlueprint(unittest.TestCase):
    ...
    def test_list_agents_returns_200(self):
        with patch("aetherra_hub.blueprints.agents.registry_client.get_registered_agents", return_value=[]):
            with patch("aetherra_hub.blueprints.agents.registry_client.get_orchestrator_status", return_value=None):
    ...
    def test_list_agents_returns_agents_from_registry(self):
        with patch("aetherra_hub.blueprints.agents.registry_client.get_registered_agents", return_value=_SAMPLE_AGENTS):
            with patch("aetherra_hub.blueprints.agents.registry_client.get_orchestrator_status", return_value=None):
                r = self.client.get("/api/agents")
        data = json.loads(r.data)
    ...
    def test_list_agents_token_correct_returns_200(self):
        ...
        with patch("aetherra_hub.blueprints.agents.registry_client.get_registered_agents", return_value=[]):
            with patch("aetherra_hub.blueprints.agents.registry_client.get_orchestrator_status", return_value=None):
                r = self.client.get("/api/agents", headers={"X-Aetherra-Token": "secret"})
        self.assertEqual(r.status_code, 200)
...
class TestInteractiveBlueprint(unittest.TestCase):
    ...
    def _mock_interactive_sys(self, status_data=None, emotion="calm", expression="neutral"):
        """Build a mock interactive system."""
        sys_mock = MagicMock()
        sys_mock.get_status.return_value = status_data or {"running": True, "emotion": emotion}
    ...
    def test_status_unavailable_returns_503(self):
        with patch("aetherra_hub.blueprints.interactive._get_interactive_system", return_value=None):
    ...
    def test_status_available_returns_200(self):
        ...
        with patch("aetherra_hub.blueprints.interactive._get_interactive_system", return_value=mock_sys):
            r = self.client.get("/api/interactive/status")
        self.assertEqual(r.status_code, 200)
        data = json.loads(r.data)
    ...
    def test_status_contains_data(self):
        mock_sys = self._mock_interactive_sys(status_data={"running": True, "emotion": "curious"})
        with patch("aetherra_hub.blueprints.interactive._get_interactive_system", return_value=mock_sys):
            r = self.client.get("/api/interactive/status")
        data = json.loads(r.data)
    ...
    def test_trigger_no_auth_key_configured_allows_request(self):
        ...
        with patch("aetherra_hub.blueprints.interactive._get_interactive_system", return_value=mock_sys):
            r = self.client.post("/api/interactive/trigger",
                                  json=payload,
...
class TestQuantumBlueprint(unittest.TestCase):
    ...
    def test_quantum_snapshot_returns_200(self):
        with patch("aetherra_hub.services.registry_client.get_memory_quantum_status",
